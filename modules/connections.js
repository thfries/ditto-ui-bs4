...
function callConnectionsAPI(params, successCallback, connectionId) {
    if (env() === 'things' && !getCurrentEnv().solutionId) { showError(null, 'Error', 'SolutionId is empty'); return; };
    $.ajax(getCurrentEnv().api_uri + params.path.replace('{{solutionId}}', getCurrentEnv().solutionId).replace('{{connectionId}}', connectionId), {
        type: params.method,
