...
export function refreshPolicy() {
    ...
    if (policyId === '') { Main.showError(null, 'Error', 'policyId is empty'); return; }
...
function refillPolicySubjectsAndRessources() {
    ...
    for (let key of Object.keys(thePolicy.entries[thePolicyEntry].subjects)) {
        Main.addTableRow($('#policySubjectsTable')[0], key, JSON.stringify(thePolicy.entries[thePolicyEntry].subjects[key]));
    }
    $('#policyResourcesTable').empty();
    for (let key of Object.keys(thePolicy.entries[thePolicyEntry].resources)) {
        Main.addTableRow($('#policyResourcesTable')[0], key, JSON.stringify(thePolicy.entries[thePolicyEntry].resources[key]));
    }
};
...
function modifyPolicyEntry(type, key, value) {
    if (thePolicyEntry && key) {
        if (value) {
            $.ajax(getCurrentEnv().api_uri + '/api/2/policies/' + thePolicy.policyId + '/entries/' + thePolicyEntry + type + key, {
                type: 'PUT',
                contentType: 'application/json',
                data: value,
                success: refreshPolicy,
            ...
            $.ajax(Environments.getCurrentEnv()().api_uri + '/api/2/policies/' + thePolicy.policyId + '/entries/' + thePolicyEntry + type + key, {
                type: 'DELETE',
                success: refreshPolicy,
                error: Main.showError
            })
